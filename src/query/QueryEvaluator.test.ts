...
describe('QueryEvaluator', () => {
  ...
  it('throw on unknown operator', () => {
    const parsedQuery = evaluator.parse('tags @> "my-service" and tags @> "foo" == 42');

    expect(() => {evaluator.evaluate(parsedQuery, {tags: []})}).toThrow('unknown node type');
  });

  it('throw on invalid syntax', () => {
    expect(() => {evaluator.parse('tags @> "my-service')}).toThrow('Unclosed quote after \"my-service\"');
  });

  it('evaluates common queries', () => {
    const query = "(tags @> 'my-service' || tags @> 'my-service-slo') && tags @> 'generated'";
    const parsedQuery = evaluator.parse(query);

    expect(evaluator.evaluate(parsedQuery, { tags: ['my-service', 'generated'] })).toBe(true);
    expect(evaluator.evaluate(parsedQuery, { tags: ['my-service-slo', 'generated'] })).toBe(true);
    expect(evaluator.evaluate(parsedQuery, { tags: ['my-service-slo', 'not-generated'] })).toBe(false);
  });
  ...
  it('evaluates queries correctly', () => {
    ...
    testCases.forEach(testCase => {
      ...
      testCase.examples.forEach(scenario => {
        expect(evaluator.evaluate(parsedQuery, scenario.context)).toBe(scenario.expectedResult);
      })
    })
  });
});
